...
function parseTenantMap(raw: string | undefined): TenantMap {
  ...
  try {
    ...
    console.warn("[tenantRouting] VOLTO_TENANT_MAP is not valid JSON; ignoring.", e);
  }
  return {};
}
...
function resolveTenantPath(hostname: string, tenantMap: TenantMap): string | null {
...
export function tenantRoutingMiddleware(rawTenantMap?: string) {
  const tenantMap = parseTenantMap(rawTenantMap ?? process.env.VOLTO_TENANT_MAP);

  return function tenantRouting(req: Request, _res: Response, next: NextFunction) {
